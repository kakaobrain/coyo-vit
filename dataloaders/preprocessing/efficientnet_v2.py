...
from . import efficientnet_v1 as preprocess_legacy
...
def preprocess_for_train(image,
    ...
    if augname:
        ...
        # image = autoaugment.distort_image(image, augname, ra_num_layers,ra_magnitude)
        if augname == 'autoaug':
            image = autoaugment.distort_image_with_autoaugment(image, autoaug_policy)
        elif augname == 'randaug':
            image = autoaugment.distort_image_with_randaugment(image, ra_num_layers, ra_magnitude)
        else:
            raise ValueError('Invalid augname: {}'.format(augname))
...
def preprocess_image(image,
    ...
    if augname and augname.startswith('ft'):
        image = preprocess_for_finetune(image, image_size, is_training, augname,
                                        ra_num_layers, ra_magnitude, autoaug_policy)
    else:
        if is_training:
            image = preprocess_for_train(image, image_size, augname, ra_num_layers,
                                         ra_magnitude, autoaug_policy)
        else:
