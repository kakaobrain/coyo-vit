# https://github.com/google/automl/blob/master/efficientnetv2/preprocess_legacy.py
...
def preprocess_for_train(image_bytes,
    ...
    if augment_name:
        ...
        if augment_name == 'autoaug':
            ...
            image = autoaugment.AutoAugment('v0').distort(image)
        elif augment_name == 'randaug':
            # image = autoaugment.distort_image_with_randaugment(image, randaug_num_layers, randaug_magnitude)
            image = autoaugment.RandAugment(randaug_num_layers, randaug_magnitude).distort(image)
        else:
